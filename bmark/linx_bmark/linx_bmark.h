...
#define ALLOC_REQ        0x287a
#define ALLOC_FINI       0x287b
...
void
destroy_test_slave(LINX *linx, LINX_SPID test_slave);

int
attach_test(LINX *linx, const char *path, int cnt, LINX_SPID server_master,
            int use_linx_api, int use_pthreads, int use_linx_s_alloc);
...
linx_bmark_burst(LINX *linx, const char *path, int cnt,
                 size_t start_msg_size, size_t end_msg_size,
                 int iterations, uint32_t burst_cnt, LINX_SPID server,
                 int use_linx_api, int use_pthreads, int use_linx_s_alloc);

int
linx_bmark_throughput(const char *path, int cnt, size_t msg_size,
                      LINX_SPID server_master, int use_linx_api,
                      int tp_instances, int use_pthreads, int use_linx_s_alloc);
#endif
