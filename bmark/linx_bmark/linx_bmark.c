/*
 * Copyright (C) 2006-2018 by Enea Software AB.
 * All rights reserved.
 *
...
/*
 * The information is retreived from /proc/stat and here
 ...
 */
void
print_cpu_load(unsigned int *starttimes, unsigned int *stoptimes)
{
   unsigned int total;
   double load;
   int i;
...
LINX_SPID
create_test_slave(LINX * linx, LINX_SPID server, int use_linx_api,
                  int use_pthreads, int use_linx_s_alloc)
{
   ...
   sig->connEst.use_linx_s_alloc = htonl(use_linx_s_alloc);
...
void
destroy_test_slave(LINX * linx, LINX_SPID ts)
{
   union LINX_SIGNAL *sig;
   LINX_SIGSELECT any_sig[] = { 0 };
...
static void
test_with_linxlib(LINX_SPID client, LINX * linx, int use_linx_s_alloc)
{
   LINX_SPID sender;
   ...
   for (;;)
   {
      ...
      switch (sig->sigNo)
      {
         ...
         case ATTACH_TEST_REQ:
            /* just close the endpoint;
             * the requester will get the att_sig as fast as possible. */
            linx_free_buf(linx, &sig);
            linx_close(linx);
            return;
         ...
         case ALLOC_REQ:
            {
               int sigCnt = ntohl(sig->allocReq.n);
               int reply_size = ntohl(sig->allocReq.reply_size);
               LINX_SIGSELECT reply_sigNo = ntohl(sig->allocReq.reply_sigNo);
               ...
               while (sigCnt--)
               {
                  ...
                  {
                     sig2 = linx_s_alloc(linx, reply_size, reply_sigNo, 0);
                  }
                  else
                  {
                     sig2 = linx_alloc(linx, reply_size, reply_sigNo);
                  }
                  sig2->burstSig.sigNo = reply_sigNo;
                  sig2->burstSig.next = list;
                  list = sig2;
               }
               sig->sigNo = ALLOC_FINI;
               linx_send(linx, &sig, client);
         ...
         case BURST_REQ:
            while (list)
            {
               ...
               sig2->burstSig.next = NULL;
               linx_send(linx, &sig2, client);
            }

            /* Free the received BURST_REQ signal. */
            linx_free_buf(linx, &sig);
            break;
...
static void
test_without_linxlib(LINX_SPID client, LINX *linx)
{
   int len, sig_len;
   ...
   for (;;)
   {
      len = sig_len = recvfrom(sd, sig, msg_size,
                               0, (void *)&to, &socklen);
      ...
      switch (sig->sigNo)
      {
         case ECHO_SIG:
            ...
            len = sendto(sd, sig, len, 0, (void *)&to, socklen);
         ...
         case ALLOC_REQ:
            sigCnt = ntohl(sig->allocReq.n);
            reply_size = ntohl(sig->allocReq.reply_size);
            reply_sigNo = ntohl(sig->allocReq.reply_sigNo);

            /* Send back the reply to inform requester that we can start the burst */
            sig->allocReq.sigNo = ALLOC_FINI;
            len = sendto(sd, sig, sizeof(struct allocReq), 0,
                         (void *)&to, socklen);

            if (unlikely(sizeof(struct allocReq) != (uint32_t)len))
            {
         ...
         case BURST_REQ:
            {
               ...
               while (sigCnt--)
               {
                  len = sendto(sd, sig, reply_size, 0,
                               (void *)&to, socklen);

                  if (unlikely(reply_size != (uint32_t)len))
                  {
                     ERR("sendto returned: %d when "
                         "asked for: %d", len,
                         reply_size);
...
test_slave_proc(void *input)
{
   ...
   sig->connEst.use_linx_s_alloc = htonl(arg_->use_linx_s_alloc);

   linx_send(linx, &sig, arg_->client);

   if (arg_->use_linx_api)
   {
      test_with_linxlib(arg_->client, linx, arg_->use_linx_s_alloc);
   }
   else
   {
...
server(void *server_name)
{
   ...
   for (;;)
   {
      ...
      if (sig->sigNo == CONN_EST)
      {
         uint32_t use_linx_api = ntohl(sig->connEst.use_linx_api);
         uint32_t use_pthreads =  ntohl(sig->connEst.use_pthreads);
         uint32_t use_linx_s_alloc = ntohl(sig->connEst.use_linx_s_alloc);
         ...
         if (use_pthreads)
         {
            ...
            struct slave_args *input = malloc(sizeof(*input));
         ...
         else
         {
            ...
            if (!pid)
            {
               ...
               if (!pid)
               {
                  ...
                  arg_.use_linx_s_alloc = use_linx_s_alloc;
...
static void
print_usage(const char *cmd_name)
{
   printf("Usage:\n  %s -S<name>\n", cmd_name);
   printf("    Start server master using <name> as the hunt name.\n\n");

   printf("  %s -a [ -u ] [ -l ] [ -p<path> ] [ -n<loop_cnt> ]\n\n", cmd_name);
   printf("    Run attach test:\n"
          "    1) A connection is established to a slave process.\n"
          "    2) The slave is terminated, which triggers an attach\n"
          "       signal.\n"
          "    Use -u to choose the linx_s_alloc instead of linx_alloc\n"
          "    Use -l to choose the LINX API instead of LINX SOCKET interface\n"
...
int main(int argc, char *argv[])
{
   ...
   while ((c = getopt(argc, argv, "S?p:auc:n:m:b:i:s:e:Alt:qPL:")) != -1)
   {
      switch (c)
      {
         case 'S':
         ...
         case 'L':
            /* Location for the measurings to be used at performance monitoring */
            outfile = optarg;
            log_perf_monitor = 1;
         break;
   ...
   if (server_name != NULL)
   {
      if (use_pthreads)
      {
         if (pthread_create(&server_th, NULL, server, (void *)server_name))
         {
            fprintf(stderr, "pthread_create(): %d\n", errno);
            exit(errno);
         }
   ...
   /* Remove the existing measurings file */
   if (log_perf_monitor) {
      if (!kill_server)
      {
         if( remove(outfile) == 0 )
   ...
   if (run_attach_test)
   {
      attach_test(linx, path, loop_cnt, server_spid,
                  use_linx_api, use_pthreads, use_linx_s_alloc);
   }
