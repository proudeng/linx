		   ...
		   uint32_t buffer_type)
{
	...
	linx_debug(LINX_TRACEGROUP_TRACE_MSG,
        ...
        if (BUF_TYPE_SKB(buffer_type)) {
                ...
                if (BUF_TYPE_KERN(buffer_type) || BUF_TYPE_POOL(buffer_type)) {
                        memcpy(skb_data_buffer, buf, size);
                } else {
                        LINX_ASSERT(BUF_TYPE_USER(buffer_type));
                        if (unlikely(copy_from_user(skb_data_buffer, buf, size) != 0)) {
                                kfree_skb(skb);
                                linx_err("linx_message_trace: failed to copy_from_user");
                                goto out;
                        }
                }
        }
...
int linx_message_trace_exit(void)
{
	...
	/* Traverse the list to make sure the device is not already registered. */
...
dev_notification(struct notifier_block *nb, unsigned long evt, void *dv)
{
	...
	linx_debug(LINX_TRACEGROUP_TRACE_MSG,
		   "evt %lu dv:0x%p device_name:%s flags:0x%x type:%hu",
		   evt, dev, dev->name, dev->flags, dev->type);
