...
/* If a spid has a value that cannot be masked away completely by both the
...
/* In Linux 4.4 some socket flags are moving from one struct to another,
 * changing their name in the process. The following comment accompanies
 * the change in the kernel 4.4:
 */
...
#else /* RHEL_RELEASE_CODE < RHEL_RELEASE_VERSION(7,2) */
	return sock_alloc_send_pskb(sk, 0, data_len, 0, errcode);
#endif
#else /* !defined RHEL_RELEASE_CODE */
	return sock_alloc_send_pskb(sk, 0, data_len, 0, errcode);
#endif
#else /* >= KERNEL_VERSION(3,12,0) */
	return sock_alloc_send_pskb(sk, 0, data_len, 0, errcode, 0);
#endif
}
...
/* __GFP_WAIT was removed in Linux 4.4 */
#ifndef __GFP_WAIT
...
#endif
{
	...
	if (unlikely(to == NULL)) {
		 ...
		 * shall be silent. The user detects this by checking the
	...
	/* If the sender is not bound, the signal cannot be sent. */
...
static int linx_ioctl_legacy_receive(struct sock *sk, unsigned long arg)
{
	 ...
	 * cannot hold the datagram, the signal is trucated and the truncated
	 * data is lost.
	 */
	cb = (struct linx_skb_cb *)skb->cb;
...
static int linx_ioctl_legacy_3_send(struct sock *sk, unsigned long arg)
{
	uint32_t from_spid, to_spid, size, sig_attr;
	uint32_t buffer_type = BUFFER_TYPE_USER;
	void *buffer;
	...
	const struct linx_sndrcv_legacy_3_param __user *sndrcv =
		(struct linx_sndrcv_legacy_3_param *)arg;

	LINX_ASSERT(access_ok(VERFIFY_READ, sndrcv,
				sizeof(struct linx_sndrcv_legacy_3_param)));
...
static int linx_ioctl_legacy_3_receive(struct sock *sk, unsigned long arg)
{
	uint32_t from_spid, size, sigselect_size, sig_attr;
	unsigned long buffer;
	struct iovec to;
		...
		case 2:
			{
				...
				put_uptr(0, (unsigned long long *)&sndrcv->buffer);
				put_user(real_size, (uint32_t *)&sndrcv->real_size);
				put_user(0, (uint32_t *)&sndrcv->next_sig_offset);
				break;
			}
		default:
			break;
...
#endif
{
	...
	if (is_pool_signal(skb->data)) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,19,0))
		if (cb->pass_ptr == 1) {
			/* If it's the same process only the pointer to the old buffer is copied */
			ret = copy_to_iter(skb->data, sizeof(void *), &msg->msg_iter);
		} else {
		...
		if (cb->pass_ptr == 1) {
			/* If it's the same process only the pointer to the old buffer is copied */
			ret = memcpy_toiovec(msg->msg_iov, skb->data, sizeof(void *));
			if (ret == 0) {
				ret = sizeof(void *);
			}
		} else {
...
#endif
		put_user(attref, (int __user *)
				&((struct linx_attach_legacy_param *)arg)->attref);
	return 0;
}
...
#endif
		put_user(tmoref, (int __user *)
				&((struct linx_tmo_legacy_param *)arg)->tmoref);
	return 0;
}
...
static int linx_ioctl_info(struct sock *sk, unsigned long arg, int compat)
{
	...
	case LINX_INFO_RECV_BUFFER_SIZE:
		return linx_ioctl_info_recv_buffer_size(&info, compat);
		break;
...
static int linx_ioctl_free(struct sock *sk, unsigned long arg)
{
	...
	ptr = (char *)get_pool_base_addr() + free_param.offset;
	free_mem(ptr, sk);

	return 0;
}

static int linx_ioctl_legacy_pool_info(struct sock *sk, unsigned long arg)
{
	uint32_t pool_size;
	uint32_t headroom;
	uint16_t offset;

	const struct linx_pool_info_legacy __user *pool_info =
		(struct linx_pool_info_legacy *)arg;
...
{
	uint32_t pool_size, headroom;
	uint32_t s_alloc_count, s_free_count;
	uint16_t offset, lists_no;
	...
	headroom = get_headroom_size();
	offset = get_pool_offset();
	s_alloc_count = get_pool_s_alloc_count();
	s_free_count = get_pool_s_free_count();
	lists_no = get_pool_lists_info(list_info, MAX_POOL_LISTS);

	put_user(pool_size, (uint32_t*)&pool_info->size);
	put_user(headroom, (uint32_t *)&pool_info->headroom);
	put_user(offset, (uint16_t *)&pool_info->offset);
	put_user(s_alloc_count, (uint32_t*)&pool_info->s_alloc_count);
	put_user(s_free_count, (uint32_t *)&pool_info->s_free_count);
	put_user(lists_no, (unsigned short *)&pool_info->num_lists);

	for(idx = 0; idx < lists_no; idx++) {
		put_user(list_info[idx].block_size,
				(unsigned long *) &pool_info->list[idx].block_size);
	}
...
static int linx_ioctl_legacy_pool_statistics(struct sock *sk, unsigned long arg)
{
	size_t pool_size;
	...
	put_user(pool_size, (__u32 *)&pool_info->size);
	put_user(headroom, (__u32 *)&pool_info->headroom);
	put_user(free_size, (__u32 *)&pool_info->free);
	put_user(total_overhead, (__u32 *)&pool_info->overhead);
	put_user(alig, (__u16 *)&pool_info->alignment);
	put_user(lists_no, (__u16 *)&pool_info->num_lists);
	put_user(offset, (__u16 *)&pool_info->offset);

	for(idx = 0; idx < lists_no; idx++) {
		put_user(list_info[idx].block_size,
				(__u32 *)&pool_info->list[idx].block_size);

		put_user(list_info[idx].allocated,
				(__u32 *)&pool_info->list[idx].allocated);

		put_user(list_info[idx].available,
				(__u32 *)&pool_info->list[idx].available);
	}
...
static int linx_ioctl_pool_statistics(struct sock *sk, unsigned long arg)
{
	...
	put_user(lists_no, (__u16 *)&pool_info->num_lists);
	put_user(offset, (__u16 *)&pool_info->offset);
	put_user(s_alloc_count, (__u32 *)&pool_info->s_alloc_count);
	put_user(s_alloc_count, (__u32 *)&pool_info->s_free_count);
...
static int linx_ioctl(struct socket *sock, unsigned int cmd, unsigned long arg)
{
	...
	case LINX_IOCTL_LEGACY_3_SEND:
		err = linx_ioctl_legacy_3_send(sk, arg);
		break;

	case LINX_IOCTL_LEGACY_3_RECEIVE:
		err = linx_ioctl_legacy_3_receive(sk, arg);
		break;
	...
	case LINX_IOCTL_LEGACY_POOL_INFO:
		err = linx_ioctl_legacy_pool_info(sk, arg);
		break;
	case LINX_IOCTL_LEGACY_POOL_STATISTICS:
		err = linx_ioctl_legacy_pool_statistics(sk, arg);
		break;
...
int af_linx_init(void)
{
	...
	register_ioctl32_conversion(LINX_IOCTL_UNREGISTER_LINK_SUPERVISOR, NULL);
